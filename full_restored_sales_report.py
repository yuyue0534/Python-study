...
def build_cover(wb, report_date):
    ...
    for col in range(2, 14):
        ws.cell(row=9, column=col).fill = PatternFill(
            "solid",
            fgColor=ORANGE
        )
    ...
    set_col_width(ws, {
        get_column_letter(c): 10 for c in range(1, 15)
    })
    ...
    for i, row in enumerate(df[cols].itertuples(index=False), 2):
        ...
        for j, val in enumerate(row, 1):
            ...
            if col_name in ("销售额", "成本", "利润", "单价"):
                ...
                data_style(ws, i, j, val, bg=bg, num_format='0.00"%"')
...
def build_overview(wb, df):
    ...
    for (title, val, color), start_col in zip(kpis, card_cols):
        end_col = start_col + 3
    ...
    for j, h in enumerate(headers, 1):
        header_style(ws, r0 + 1, j, h)

    for i, row in enumerate(q_data.itertuples(index=False), r0 + 2):
        data_style(ws, i, 1, row.季度)
        data_style(ws, i, 2, row.销售额, num_format='#,##0')
        data_style(ws, i, 3, row.利润, num_format='#,##0')
        data_style(ws, i, 4, row.订单数)
...
def build_region(wb, df):
    ...
    for j, h in enumerate(headers, 1):
        header_style(ws, 2, j, h)

    for i, row in enumerate(region_df.itertuples(index=False), 3):
        data_style(ws, i, 1, row.地区)
        data_style(ws, i, 2, row.销售额, num_format='#,##0')
        data_style(ws, i, 3, row.利润, num_format='#,##0')
        data_style(ws, i, 4, row.订单数)
        data_style(ws, i, 5, round(row.平均利润率, 1))
...
def build_product(wb, df):
    ...
    headers = ["产品", "销售额", "利润", "销量", "订单数"]

    for j, h in enumerate(headers, 1):
        header_style(ws, 2, j, h)

    for i, row in enumerate(prod_df.itertuples(index=False), 3):
        data_style(ws, i, 1, row.产品)
        data_style(ws, i, 2, row.销售额, num_format='#,##0')
        data_style(ws, i, 3, row.利润, num_format='#,##0')
        data_style(ws, i, 4, row.数量)
        data_style(ws, i, 5, row.订单数)
...
def build_ranking(wb, df):
    ...
    for rank, row in rep_df.iterrows():
        bg = {
            ...
            rank,
            WHITE if rank % 2 == 0 else GRAY_LIGHT
        )

        data_style(ws, rank + 2, 1, rank, bg=bg)
        data_style(ws, rank + 2, 2, row["销售员"], bg=bg)
        data_style(
            ws,
            rank + 2,
            3,
...
def self_check(wb, df):
    ...
    if raw_ws.max_row - 1 != len(df):
        issues.append(
            f"原始数据行数不匹配: "
            f"期望 {len(df)}, 实际 {raw_ws.max_row - 1}"
        )
...
def main():
    ...
    output_path = (
        f"销售自动化报表_2024_{timestamp}.xlsx"
    )
